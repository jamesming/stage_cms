...
form#image_showpage_episodes_item_form{
font-size:16px;
}
form#image_showpage_episodes_item_form input[type=text]{
padding:6px 5px;
width:490px;	
}
form#image_showpage_episodes_item_form #short_version{
width:490px;	
}
form#image_showpage_episodes_item_form table#main {
width:100%;
margin:30px 0px 0px 0px;	
}
form#image_showpage_episodes_item_form table#main td.main_table{
padding-top:5px;
padding-bottom:5px;	
}
...
form#image_showpage_episodes_item_form #submit{
width:70px;	
}
...
<body >
<form id='image_showpage_episodes_item_form'>
		<table  id='main'>
			<tr>
				<td  class='main_table ' > Name
				</td>
				<td  class='main_table '><input name="name" id="" type="text" value="<?php echo $data['showpage_episodes_items'][0]['name']    ?>">
				</td>
			</tr>
			<tr>
				<td  class='main_table '> Title
				</td>
				<td  class='main_table '><input name="title" id="" type="text" value="<?php echo $data['showpage_episodes_items'][0]['title']    ?>">
				</td>
			</tr>
			...
			<tr>
				...
				<td   class='main_table ' colspan=2>
					<div>
							<textarea  class=' clearfix' name='short_version' id='short_version'><?php echo $data['showpage_episodes_items'][0]['short_version']    ?></textarea>
					</div>
				</td>
			</tr>	
			...
			<tr>
				<td   class='main_table ' colspan=2>
					<div  id='textarea_div'   >
							<textarea  class=' clearfix' name='content' id='text_area'><?php echo $data['showpage_episodes_items'][0]['content']    ?></textarea>
					</div>
				</td>
			</tr>	
	...
	$(document).ready(function() {
				...
				$('.image_div').css({cursor:'pointer'}).click(function(event) {
					...
					open_dialogue_upload_image(
					 $(this).attr('showpage_episodes_items_image_id'),
...
function submit_inputs(close_fancyzoom){
					...
					$.post("<?php echo base_url(). 'index.php/main/ajax_update';    ?>",{
						...
						crud:'update',
						set_what_array: $('#image_showpage_episodes_item_form').serialize(),
						id:'<?php echo $data['showpage_episodes_items'][0]['id']    ?>'
						},function(xml) {
...
function open_dialogue_upload_image(
 showpage_episodes_items_image_id, 
 image_type, 
 image_type_id 
 ){
		...
		if( showpage_episodes_items_image_id == null){
			showpage_episodes_items_image_id = 0;
		};

		$("#iframe_src_for_image")
		.css({width:'350px',height:'80px'})
		.attr('src','<?php echo base_url();    ?>index.php/main/upload_image_form?what_item=showpage_episodes&showpage_episodes_item_id=<?php echo $data['showpage_episodes_items'][0]['id']    ?>&showpage_episodes_items_image_id=' + showpage_episodes_items_image_id +'&image_type='+image_type +'&image_type_id='+image_type_id);
