...
body#calendar span,body#calendar caption{
background:white;	
}
table.calendar{
	margin-top:20px;
...
table.calendar td.isToday{
	background:#EBE8ED;
}

table.calendar td div.top_of_td_div{
	margin-bottom:25px;
}	
		table.calendar td div.top_of_td_div div{
			width:20px;
			height:20px;
		}	
		table.calendar td div.top_of_td_div div.day_numeric{
			margin-right:83px;
		}	
		table.calendar td div.top_of_td_div div.launch_icon{
...
table.calendar td div.bottom_of_td_div{
	padding:5px;
	width:auto;
	height:60px;
}	
		table.calendar td div.bottom_of_td_div div.nu_spotlight_sets_booked{
			clear:both;
			text-align:center;
			height:25px;
			padding-top:5px;
		...
		table.calendar td div.bottom_of_td_div div.nu_spotlight_videos_booked{
			clear:both;
			text-align:center;
			height:25px;
			padding-top:5px;
...
$(document).ready(function() {
	...
	$('table.calendar td div.top_of_td_div div.launch_icon').each(function(e) {
	...
	$('table.calendar tr').children('td:nth-child(1)').css({'border-left':'1px solid darkgray'});
	
	
	$('table.calendar tr td[year="<?php echo $data['year']; ?>"] div.bottom_of_td_div').each(function(e) {
		$(this).css({cursor:'pointer'}).attr('nu_spotlight_set_id',0).attr('href','#fancy_zoom_div').fancyZoom().click(function(event) {
			$("#iframe_src").attr('src','<?php echo base_url();    ?>index.php/main/line_up_form?nu_spotlight_videos_calendar_id=' + $(this).attr('nu_spotlight_videos_calendar_id') + '&nu_spotlight_set_id='  +  $(this).attr('nu_spotlight_set_id') + '&month=' + $(this).parent().attr('month') +'&day=' + $(this).parent().attr('day') + '&year=' + $(this).parent().attr('year') );
		});
	});	
	
	<?php  foreach( $data['nu_spotlight_sets_calendars']  as  $nu_spotlight_sets_calendar){ ?>
		
		$('table.calendar tr td[day="<?php echo $nu_spotlight_sets_calendar->day    ?>"] div.bottom_of_td_div')
	...
	<?php  foreach( $data['nu_spotlight_videos_calendars']  as  $nu_spotlight_videos_calendar){ ?>
		...
		$('table.calendar tr td[day="<?php echo $nu_spotlight_videos_calendar->day    ?>"] div.bottom_of_td_div')
		.attr('nu_spotlight_videos_calendar_id',<?php echo $nu_spotlight_videos_calendar->nu_spotlight_videos_calendar_id    ?>).append('<div  class="nu_spotlight_videos_booked " ><?php echo $nu_spotlight_videos_calendar->title    ?></div>')
	...
	<?php if( $data['month'] == date("m")){
			...
			for($i=1; $i <= $data['today'] - 1; $i++ ){?>
				...
				$('table.calendar tr td[day="<?php echo $i    ?>"]')
	...
	<?php if( $data['month'] < date("m")){?>
				...
				$('table.calendar tr td[year="<?php echo $data['year']; ?>"]')
				.css({background:'beige'})
	<?php
	} 
	?>	
